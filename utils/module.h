...
public:
	...
	void *ResolveRelativeAddress(void *addr, const ptrdiff_t registerOffset, const ptrdiff_t nextInstructionOffset)
	{
		uint8_t* byteAddr = reinterpret_cast<uint8_t*>(addr);
		uint8_t* skipRegister = byteAddr + registerOffset;
		int32_t relativeAddress = *reinterpret_cast<int32_t*>(skipRegister);
		uint8_t* nextInstruction = byteAddr + nextInstructionOffset;
	...
	void* AddOffset(void* addr, int offset) 
	{
		return reinterpret_cast<uint8_t*>(addr) + offset;
	}

	void* ToAbsolute(void* addr, int preOffset, int postOffset) 
	{
		uint8_t* byteAddr = reinterpret_cast<uint8_t*>(addr);
		byteAddr += preOffset;
		int32_t relativeAddress = *reinterpret_cast<int32_t*>(byteAddr);
		byteAddr += sizeof(int32_t) + relativeAddress + postOffset;
		return byteAddr;
	}

	void* FindAndResolvePattern(const byte* pattern, int preOffset, int postOffset)
	{
		void* addr = FindSignature(pattern);
		if (addr)
		{
			return ToAbsolute(addr, preOffset, postOffset);
		}
		return nullptr;
	}
